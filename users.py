...
async def auto_signup(user):
    if not does_user_exist(user.id):
        user_id = user.id
        nickname = user.name
...
async def manual_signup(message):
    ...
    if text[0] == 'mk' and text[1] == 'user' and len(text) == 4:
            user_id = text[2]
            nickname = text[3]
            if main.verid(user_id):
                await create_new_user(user_id, nickname)
        else:
            main.chide(message, 'mk user <id> <nickname>')

async def create_new_user(user_id, nickname):
    if not does_user_exist(user_id):
        db.update("INSERT INTO user_log (user_id, nickname) VALUES('{user_id}', '{nickname}';".format(user_id, nickname))
...
async def manual_change_balance(message):
    ...
    if text[0]=='ch' and text[1]==' bal' and len(text) == 4:
        delta = text[3]
        user_id = text[2]
        if str_is_signum_int(delta):
            delta = signumintstr_to_int(delta)
            await change_balance(user_id, delta)
    else:
        main.chide(message, 'ch bal <id> <+/-><integer>')
...
async def manual_set_balance(message):
    ...
    if text[0]=='set' and text[1]=='bal' and len(text) == 4:
        bal = text[3]
        user_id = text[2]
        if str_is_signum_int(bal):
            bal = signumintstr_to_int(bal)
            await set_balance(user_id, bal)
    else:
        main.chide(message, 'set bal <id> <+/->integer')
